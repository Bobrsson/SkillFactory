...
from extensions import ConvertExeption, CryptoConvertor
...
def help(message: telebot.types.Message):
    text = 'Чтобы начать работу с данным ботом, введите команду в следующем формате: \n<имя валюты> \
     ...
     <кол-во переводимой валюты>\n Увидеть список доступных для конверта валют : /values'
...
def convert(message: telebot.types.Message):
    ...
    try:
        if len(values) != 3:
            raise ConvertExeption('НЕправельное количество параметров.')
